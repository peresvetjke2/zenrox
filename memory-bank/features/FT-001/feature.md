...
purpose: "Расширенный canonical feature-документ для первого capture slice: сохранить одну личную задачу из одной текстовой реплики либо явно отказать без ложного подтверждения."
derived_from:
  - ../../domain/problem.md
...
`PRD-001` задает первый полезный task loop для `zenrox`, но сам по себе не определяет delivery-scoped contract capture-path. Для первого MVP нужен узкий и trustworthy slice, в котором пользователь может отправить одну короткую текстовую реплику и получить либо одну сохраненную `open`-задачу, либо явный отказ с объяснением причины.

Эта фича сознательно не пытается решать general parsing problem. Ее задача — зафиксировать минимальный admission rule для первого capture-flow так, чтобы `zenrox` не создавал молчаливые побочные записи, не подтверждал неуспешное сохранение и не размывал первый MVP до multi-intent или mixed-intent интерпретации.
...
- `REQ-03` Если реплика не проходит admission rule первого MVP, система не выполняет автосохранение и дает user-visible обратную связь, что именно помешало сохранить задачу автоматически и какое уточнение требуется.
...
- `NS-04` Глубокий extraction дополнительных атрибутов, сложная нормализация текста и parsing expansion beyond admission rule.
...
Фича вводит узкий capture admission layer перед сохранением задачи. Система принимает только те текстовые реплики, которые можно безопасно интерпретировать как одну личную задачу, и во всех остальных случаях возвращает explanatory rejection вместо рискованного автосохранения. Подтверждение о сохранении допустимо только после фактического успешного завершения write-path.
...
| `CTR-03` | `unsupported or failed capture -> explanatory non-success response` | capture-path / write path -> user-facing response | Ответ должен объяснять, что именно помешало автосохранению или что сохранение не завершилось |
...
- `EC-02` Unsupported input не приводит к автосохранению и сопровождается явной explanatory feedback о причине отказа и требуемом уточнении.
...
| --- | --- | --- | --- | --- |
| `REQ-01` | `CON-01`, `CON-02`, `CTR-01` | `EC-01`, `SC-01` | `CHK-01` | `EVID-01` |
| `REQ-02` | `CON-03`, `CTR-02` | `EC-01`, `SC-01` | `CHK-01` | `EVID-01` |
| `REQ-03` | `CON-01`, `CON-03`, `CTR-03`, `FM-01`, `FM-02`, `FM-03`, `FM-05` | `EC-02`, `SC-02`, `NEG-01`, `NEG-02`, `NEG-03` | `CHK-02` | `EVID-02` |
| `REQ-04` | `CON-03`, `CTR-02`, `CTR-03`, `FM-04` | `EC-03`, `NEG-04` | `CHK-03` | `EVID-03` |
| `REQ-05` | `CON-01`, `CON-02`, `DEC-01` | `EC-01`, `EC-02`, `SC-01`, `SC-02`, `NEG-01`, `NEG-02`, `NEG-03` | `CHK-01`, `CHK-02` | `EVID-01`, `EVID-02` |
...
- `SC-02` Пользователь отправляет unsupported текстовую реплику, которая не проходит admission rule, и система не создает задачу, а возвращает явное объяснение причины и нужного уточнения.
...
| --- | --- | --- | --- | --- |
| `CHK-01` | `EC-01`, `SC-01` | Прогнать deterministic supported-input сценарий на репликах вида `купить молоко`, `позвонить маме`; проверить persisted result и user-visible response | Для каждой supported реплики создается ровно одна `open`-задача и появляется короткое подтверждение только после успешного сохранения | `artifacts/ft-001/verify/chk-01/` |
| `CHK-02` | `EC-02`, `SC-02`, `NEG-01`, `NEG-02`, `NEG-03` | Прогнать unsupported-input сценарии на ambiguity / multi-intent / non-task репликах; проверить отсутствие новой задачи и наличие explanatory rejection | Для каждой unsupported реплики новая задача не создается, ответ объясняет причину отказа и указывает, что требуется уточнение | `artifacts/ft-001/verify/chk-02/` |
| `CHK-03` | `EC-03`, `NEG-04` | Смоделировать или воспроизвести неуспешный write-path и проверить user-visible response | При write failure нет success-like подтверждения; пользователь получает явно неуспешный результат | `artifacts/ft-001/verify/chk-03/` |
...
- `EVID-03` Артефакт failed-save сценария, подтверждающий отсутствие ложного подтверждения.
